/* Polaris Concierge | hero-bg.js v5
   Dark dotted world map + elegant flight route lines
   Inspired by: gauss.ag style — but dark, luxury, minimal
*/
(function(){
'use strict';

/* ── Continent outlines (lon/lat pairs) ── */
var LAND=[
  /* North America */
...
var ROUTES=[
  ...
  {a:4,b:2,arc:0.20},  /* GVA-DXB */
  {a:5,b:1,arc:0.26},  /* MIA-LON */
  {a:1,b:6,arc:0.24},  /* LON-SIN */
  {a:0,b:2,arc:0.30},  /* NYC-DXB */
  {a:2,b:7,arc:0.18}   /* DXB-TYO */
];
...
function setup(){
  ...
  /* Control points for arcs (curve upward/northward) */
  ...
  projLand=LAND.map(function(p){
    return p.map(function(q){return proj(q[0],q[1]);});
  });

  /* Build dot grid */
  dots=[];
  var S=6; /* dot spacing px */
...
function buildStatic(){
  ...
  /* ── Dot matrix: the continent map ──
     Warm gray dots, clearly visible on black
  ── */
  sc.fillStyle='rgba(168,158,138,0.82)';
  sc.beginPath();
  for(var d=0;d<dots.length;d+=2){
    var dx=dots[d], dy=dots[d+1];
    sc.moveTo(dx+1.8,dy);
    sc.arc(dx,dy,1.8,0,Math.PI*2);
  }
  sc.fill();

  /* Second pass: brighter accent dots for subtle depth */
  sc.fillStyle='rgba(200,190,168,0.22)';
  sc.beginPath();
  for(var d=0;d<dots.length;d+=4){
    var dx=dots[d], dy=dots[d+1];
    sc.moveTo(dx+1.8,dy);
    sc.arc(dx,dy,1.8,0,Math.PI*2);
  }
  ...
  /* ── Edge vignette: fade edges to black, keep center open ── */
  /* Radial: generous inner zone */
  var vg=sc.createRadialGradient(W/2,H/2,H*0.28,W/2,H/2,W*0.72);
  vg.addColorStop(0,'rgba(8,8,7,0)');
  vg.addColorStop(0.60,'rgba(8,8,7,0.05)');
  vg.addColorStop(0.82,'rgba(8,8,7,0.45)');
  vg.addColorStop(1,'rgba(8,8,7,0.92)');
  sc.fillStyle=vg;
  sc.fillRect(0,0,W,H);

  /* Left */
  var lf=sc.createLinearGradient(0,0,W*0.08,0);
  lf.addColorStop(0,'rgba(8,8,7,0.96)');
  lf.addColorStop(1,'rgba(8,8,7,0)');
  sc.fillStyle=lf; sc.fillRect(0,0,W*0.08,H);

  /* Right */
  var rf=sc.createLinearGradient(W*0.92,0,W,0);
  rf.addColorStop(0,'rgba(8,8,7,0)');
  rf.addColorStop(1,'rgba(8,8,7,0.96)');
  sc.fillStyle=rf; sc.fillRect(W*0.92,0,W*0.08,H);

  /* Bottom — keeps text readable */
  var bf=sc.createLinearGradient(0,H*0.72,0,H);
  bf.addColorStop(0,'rgba(8,8,7,0)');
  bf.addColorStop(1,'rgba(8,8,7,0.95)');
  sc.fillStyle=bf; sc.fillRect(0,H*0.72,W,H*0.28);

  /* Top — behind nav */
  var tf=sc.createLinearGradient(0,0,0,H*0.18);
  tf.addColorStop(0,'rgba(8,8,7,0.72)');
  tf.addColorStop(1,'rgba(8,8,7,0)');
  sc.fillStyle=tf; sc.fillRect(0,0,W,H*0.18);
  ...
  for(var j=0;j<CITIES.length;j++){
    ...
    g.addColorStop(0,'rgba(200,185,155,0.18)');
    g.addColorStop(1,'rgba(200,185,155,0)');
    sc.fillStyle=g;
    sc.beginPath(); sc.arc(cx,cy,18,0,Math.PI*2); sc.fill();

    /* Dot */
    sc.fillStyle='rgba(210,198,175,0.90)';
    sc.beginPath(); sc.arc(cx,cy,2.5,0,Math.PI*2); sc.fill();

    /* Label */
    sc.save();
    sc.font='600 9px Archivo,sans-serif';
    if('letterSpacing' in sc)sc.letterSpacing='0.15em';
    sc.fillStyle='rgba(180,168,145,0.55)';
    sc.textAlign='center';
    sc.fillText(CITIES[j].id,cx,cy-13);
    sc.restore();
  }
}
...
/* ════════════════════════════════════
   ...
   Clean bezier line from city to city.
   Leading glowing point + fading line behind it.
════════════════════════════════════ */
function launch(now){
  trails.push({
    ri: nextRoute++ % ROUTES.length,
    born: now,
    dur: 18000 + Math.random()*10000  /* 18-28s — slow, cinematic */
  });
  lastLaunch=now;
}
...
function drawTrail(tr,now){
  ...
  /* Opacity envelope: fade in first 5%, fade out last 6% */
  var ft = t<0.05 ? t/0.05 : t>0.94 ? (1-t)/0.06 : 1;
  ft=Math.max(0,Math.min(1,ft));

  /* Trail: draw from tail to head with gradient opacity */
  var TRAIL=0.07;
  ...
  for(var i=0;i<N;i++){
    ...
    /* Opacity: near-zero at tail, bright at head */
    var frac=i/N;
    var alpha=Math.pow(frac,0.60)*0.55*ft;
  ...
  /* Outer glow only — no harsh bright blob */
...
function frame(now){
  ...
  /* Max 2 simultaneous trails, launch every 7s */
